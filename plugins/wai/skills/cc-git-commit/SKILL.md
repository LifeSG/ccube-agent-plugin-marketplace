...
argument-hint: >-
  Optionally describe what changed, e.g. "committed frontend bug fix
  and updated docs"
user-invocable: true
---
...
Every commit run MUST produce:
- One or more `git commit` commands, each with a well-formed Conventional
  Commit message starting with `[BRANCH][INITIALS] type(scope): subject`
- Separate commits for logically distinct concerns (never batch all
  changes into one commit)
- Only files relevant to each group staged for that commit
- A final summary listing all commit SHAs and their subjects

### Feedback Sensors (MUST-NOT-contain)

Every commit run MUST NOT produce:
- A commit message that does not start with `[<BRANCH_NAME>]`
- A commit that mixes multiple unrelated concerns
- A `git push --force` or `git reset --hard` command
- Commit messages in non-imperative form (e.g. "Fixed bug" not "fix(auth): fix login")

**PASS example:**
> Input: Three changed files — new component, updated README, and a bug fix in utils.
>
> Output: Three separate commits:
> `[main][LJ] feat(ui): add SearchBar component`
> `[main][LJ] docs: update README with SearchBar usage`
> `[main][LJ] fix(utils): handle null input in formatDate`

**FAIL example:**
> Output: Single commit `[main][LJ] update: multiple changes` staging
> all three files together.
> *(Fails: mixes unrelated concerns into one commit, vague subject)*

### Test Cases (features × scenarios × personas)

| Feature          | Scenario                                   | Persona              | Expected behaviour                                                      |
| ---------------- | ------------------------------------------ | -------------------- | ----------------------------------------------------------------------- |
| Atomic grouping  | 5 files changed: 2 features, 1 fix, 2 docs | Full-stack developer | 3 commits, each covering one logical concern; no cross-concern batching |
| Message format   | Branch is `feature/ep-001-add-auth`        | Any developer        | Prefix `[feature/ep-001-add-auth][INITIALS]` applied to every commit    |
| Nothing staged   | `git status` shows no changes              | Any developer        | Informs user: "No changes to commit" — does not create empty commits    |
| Mixed file types | CSS, TS, and migration changed together    | Frontend developer   | Migration in own commit; TS grouped by feature; CSS with its component  |
