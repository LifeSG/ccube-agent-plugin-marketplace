...
Every deployment walkthrough MUST contain:
- A prerequisite check confirming git is available
- The `git remote add origin` command with `oauth2:` token
  placeholder — never the actual token
- A `git push -u origin main` command
- The expected sandbox URL pattern for the deployed app
- A security warning that the token must not be shared in chat

### Feedback Sensors (MUST-NOT-contain)

Every deployment walkthrough MUST NOT contain:
- Actual Project Access Tokens, secrets, or credentials
- `git push --force` or any destructive git operation
- Steps that skip `git init` when no `.git` directory exists
- Assumptions about branch name without checking

**PASS example:**
> Input: "Deploy my app"; workspace has no `.git` folder.
>
> Output: Runs `git init`, prompts user to paste token into
> `oauth2:<TOKEN>@...` placeholder, runs `git push -u origin main`,
> reports sandbox URL. Security warning present.

**FAIL example:**
> Output includes `git remote add origin https://oauth2:glpat-abc123@...`
> *(Fails: actual token value included in output — never allowed)*

### Test Cases (features × scenarios × personas)

| Feature          | Scenario                                | Persona                  | Expected behaviour                                                       |
| ---------------- | --------------------------------------- | ------------------------ | ------------------------------------------------------------------------ |
| First deployment | No .git directory, no remote configured | Junior developer on SEED | Full flow: git init → remote add → commit → push; sandbox URL reported   |
| Redeployment     | Existing repo with origin already set   | Returning developer      | Skips init/remote setup; stages, commits, pushes changes                 |
| Token expired    | Push fails with 401 Unauthorized        | Any developer            | Directs to portal to regenerate token; provides `git remote set-url` fix |
| Wrong branch     | Default branch is master, not main      | Developer on legacy repo | Detects branch name and adjusts push command accordingly                 |
