...
- A Mermaid dependency graph with all tasks and edges
- A phase table listing tasks, dependencies, and assigned agents
- A critical path annotation identifying the longest path
- An `Agent Prompt` section for every task with actionable
  instructions for the executing agent
- A `Definition of Done` checklist at the end of the plan

### Feedback Sensors (MUST-NOT-contain)

Every implementation plan MUST NOT contain:
- Tasks with no `Depends On` entry that actually depend on another
  task (missing edge in the graph)
- Two tasks in the same phase that modify the same file
- Circular dependencies in the task graph
- Vague agent prompts ("implement the feature") without file,
  input, and output specification

**PASS example:**
> Input: EP file describing a 3-endpoint Koa API with a React
> frontend page and database migration.
>
> Output: Phase 1 (parallel): T01 backend routes, T02 migration,
> T03 React page. Phase 2 (sequential): T04 integration test
> (depends T01+T02+T03). Mermaid graph rendered. Critical path:
> T01→T04. Each task has a specific agent prompt.

**FAIL example:**
> Output: "Phase 1: implement backend. Phase 2: implement frontend."
> *(Fails: no task IDs, no dependency graph, no agent prompts,
> no critical path)*
...
| ------------------ | ----------------------------------- | ---------------- | ------------------------------------------------------------------------ |
| Parallelisation    | EP has 4 independent frontend tasks | Tech lead        | All 4 in the same phase; no sequential dependencies introduced           |
| Conflict detection | Two tasks both modify `src/App.tsx` | Senior engineer  | Conflict flagged; tasks moved to sequential phases before plan written   |
| Dependency graph   | Task C depends on A and B           | Any engineer     | Mermaid graph shows `A --> C` and `B --> C` edges; critical path correct |
| Large EP           | 12 tasks across 3 features          | Engineering lead | Plan grouped into phases; parallel dispatch table shows 12 rows          |
