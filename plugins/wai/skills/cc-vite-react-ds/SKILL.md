...
argument-hint: >-
  Provide the project name (kebab-case) and target directory, e.g.
  "my-app /Users/username/projects"
...
After execution, the project directory MUST contain:
- `package.json` with `@lifesg/react-design-system`, `@lifesg/react-icons`,
  and `styled-components` as dependencies
...
After execution, the project MUST NOT contain:
- Raw HTML form controls in `App.tsx` (`<input>`, `<button>`,
  `<select>`)
- Legacy `ThemeProvider` (must use `DSThemeProvider`)
- Hardcoded CSS values (must use FDS tokens)
- FDS v4 imports when v3 is installed (or vice versa)

**PASS example:**
> Input: `cc-vite-react-ds my-tasks /Users/username/projects`
>
> Output: Project created at `/Users/username/projects/my-tasks`;
> `src/main.tsx` wraps app in `DSThemeProvider`; `npm run dev`
> starts without errors; no raw HTML controls in App.tsx.

**FAIL example:**
> Output: `src/App.tsx` contains `<button>Submit</button>` and
> `style={{ color: '#000' }}`.
> *(Fails: raw HTML control and hardcoded CSS value violate FDS
> constraints)*
...
| ------------------ | ------------------------------------ | ------------------------ | ----------------------------------------------------------------------- |
| Fresh scaffold     | New project with default settings    | Non-technical user       | Script completes with ✅; `npm run dev` starts; FDS-styled content shown |
| Theme verification | DSThemeProvider configured correctly | Frontend developer       | Light theme applied; no styled-components console errors                |
| Build check        | `npm run build` after scaffold       | CI pipeline              | Build succeeds; no TypeScript errors; `dist/` output created            |
| Manual fallback    | Script not found in environment      | Developer without plugin | Manual steps 1–6 replicate script output identically                    |
