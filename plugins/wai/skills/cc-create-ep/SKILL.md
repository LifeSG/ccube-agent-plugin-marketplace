...
Every EP output MUST contain:
- A `## Summary` section with a one-paragraph feature overview
- A `## Problem Statement` section with at least one pain point
- A `## Proposed Solution` section derived from the workspace
  context and user input — not from training assumptions
- The file saved at the path agreed with the user, named using
  the workspace’s existing EP naming convention
- All template section headings from the resolved template

### Feedback Sensors (MUST-NOT-contain)

Every EP output MUST NOT contain:
- Implementation details (code snippets, file names) in Part-1
  before the user confirms scope
- Sections not present in the verified template unless explicitly
  added by the user
- References to file paths or patterns that were not confirmed
  via workspace search

**PASS example:**
> Input: "Write an EP for adding Markdown export to our notes app"
>
> Output: Skill reads EP template from workspace, discovers
> existing EP naming convention, creates
> `docs/eps/EP-007-markdown-export.md` with all template sections
> populated. Halts after Part-1 for user confirmation before
> proceeding to Part-2.

**FAIL example:**
> Output: EP file created at `EP-markdown-export.md` in the repo
> root without checking the workspace’s naming convention or
> asking the user to confirm Part-1.
> *(Fails: path not confirmed via workspace search; user not
> gated at Part-1)*
...
| ------------------- | ---------------------------------------------- | ----------------- | ------------------------------------------------------------------------------- |
| New EP              | No existing EPs in workspace                   | Software engineer | Skill uses bundled template fallback; asks user to confirm Part-1 before Part-2 |
| Existing EP pattern | Workspace has EPs named `EP-NNN-kebab-title`   | Tech lead         | Naming convention auto-detected; new EP follows same pattern                    |
| Partial context     | User provides Figma URL but no problem details | Product designer  | Skill asks discovery questions before generating any content                    |
| Large EP            | Feature spans 3 user roles and 5 screens       | Senior engineer   | Part-1 covers all roles; Part-2 adds per-screen implementation notes            |
