...
Every Prompt Refiner response MUST contain:
- A `**Refined prompt:**` section with a direct imperative instruction
- A `**Prompt engineering principles applied:**` section with 2–4
  bullet points
- A `**What was improved:**` section with 2–5 bullet points
- `[placeholder]` markers for any detail that cannot be inferred

### Feedback Sensors (MUST-NOT-contain)

Every Prompt Refiner response MUST NOT contain:
- Questions or clarifying requests
- Meta-commentary about the refinement process itself
- Phrases like "could you", "please clarify", "can you confirm"
- More than 5 sentences in the refined prompt
- Content beyond the three required sections

### Example Input/Output

**PASS — structured refinement with placeholders**:
> Input: "fix the login bug"
>
> Output contains: Refined prompt with file target `[placeholder]`,
> failure description `[placeholder]`, expected outcome, and scope
> constraint. Principles section cites target-action-outcome.
> Improvements section explains each change.

**FAIL — question instead of refinement**:
> Input: "fix the login bug"
>
> Output: "Which file contains the login code? What error are you
> seeing?"
> *(Violates no-questions rule; should refine with placeholders)*

### Test Cases (features × scenarios × personas)

| Feature          | Scenario                                   | Persona               | Expected behaviour                                                   |
| ---------------- | ------------------------------------------ | --------------------- | -------------------------------------------------------------------- |
| Vague prompt     | Single-word action ("fix the bug")         | Non-technical user    | Refined with target/action/outcome structure and placeholders        |
| Multi-step task  | Comma-separated actions in one prompt      | Experienced developer | Refined into numbered sequential steps with dependency order         |
| Already specific | Detailed prompt with file, action, outcome | Senior engineer       | Skipped per invocation gate — prompt already qualifies for exception |
| Trivial input    | "yes" or "ok"                              | Any user              | Skipped per invocation gate — trivial acknowledgement exception      |
